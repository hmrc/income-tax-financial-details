...
object ClaimToAdjustPoaTestConstants {
  ...
  val claimToAdjustPoaSuccessResponse: ClaimToAdjustPoaResponse =
    ClaimToAdjustPoaResponse(
      status = CREATED,
      claimToAdjustPoaBody = Right(SuccessResponse("2024-01-31T09:27:17Z"))
    )

  val claimToAdjustPoaErrorResponse: ClaimToAdjustPoaResponse =
    ClaimToAdjustPoaResponse(
      status = Status.INTERNAL_SERVER_ERROR,
      Left(ErrorResponse("error"))
    )

  val claimToAdjustPoaRequest = ClaimToAdjustPoaRequest(
    nino = "AA1111111A", taxYear = "2025", amount = 12.00, poaAdjustmentReason = MainIncomeLower
  )
  
  val claimToAdjustPoaResult: ClaimToAdjustPoaResponse =
    ClaimToAdjustPoaResponse(
